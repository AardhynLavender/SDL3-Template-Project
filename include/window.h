...
private:
    ...
    [[nodiscard]] inline static constexpr unsigned int buildFlags(const Flags flags) {
        ...
        if (flags.fullscreenDesktop) flagInt |= SDL_WINDOW_FULLSCREEN;
        ...
        if (flags.mouseGrabbed) flagInt |= SDL_WINDOW_MOUSE_GRABBED;
        ...
        if (flags.highPixelDensity) flagInt |= SDL_WINDOW_HIGH_PIXEL_DENSITY;
