...
public:
    ...
    inline void Clear() {
        ResetColor();
        if (SDL_RenderClear(renderer)) throw SDLException(SDL_GetError());
    }

    void DrawLine(Vec2<int> a, Vec2<int> b, Color color = Colors::white);

    void DrawRect(Rec2<int> rect, Color color = Colors::white, Color fill = Colors::transparent);

    void DrawPixel(Vec2<int> vec, Color color = Colors::white);
